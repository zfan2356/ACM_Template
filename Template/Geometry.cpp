...
struct point {
    ...
    friend bool is_collinear(point P, point Q, point R) {
...
struct circle {
    ...
    bool in_circle(const point &P) const {
        return dist(P, C) <= r + EPS;
    }

    bool is_intersection(const circle &C1) const {
        double d = dist(C, C1.C);
        return fabs(r - C1.r) <= d && d <= (r + C1.r);
    }

    friend pair<point, point> circle_intersection(circle C1, circle C2) {
        double d = dist(C1.C, C2.C);
        double x = (C1.r * C1.r - C2.r * C2.r + d * d) / (2 * d);
        point A = C1.C + (C2.C - C1.C) / d * x;
        double y = sqrt(C1.r * C1.r - x * x);
        point B = (C2.C - C1.C).rotate90() / d * y;
        return {A + B, A - B};
    }
};
