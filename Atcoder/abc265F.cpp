...
Z dfs(int l, int r, vector<node> v) {
    bool ok = find(v.begin(), v.end(), (node){l, r});
    if (~f[l][r][ok]) return f[l][r][ok];
    if (l + 2 >= r) {
        if (r - l == 2) return (Z)((!ok) + 1);
        return (Z)1;
    }
    Z ans = 0;
    for (int i = l; i <= r; i ++ ) {
        for (int j = 2; j <= 10 && i + j - 1 <= r; j += 2) {
            int L = i, R = i + j - 1;
            if (ok && i == l && R == r) continue;
            for (auto [x, y] : v) {
                if (x == L && y == R) continue;
                if (x >= L && y <= R) continue;
                if (x >= R)
